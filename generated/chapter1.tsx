    ...
    import VerticalChunk from "~/components/VerticalChunk"
        ...
        const Chapter = (props: any) => {
            return <>
            <ArticleTitle label={`Chapter 1: ` + props.title } on_mobile_label={`Chapter 1: ` + props.mobile_title }/>
            {props.children}</>
        }

    const Article = () => {
    return <> <Chapter title="A Few Refreshers"><Section><VerticalChunk>{``}<b>{`Square hihi Roots.`}</b>{`
...
than `}<NoBreak><Math>{`$
0.001
$`}</Math></NoBreak>{`.)`}</VerticalChunk></Solution></Exercise></Exercises></Chapter> </>
  }
